...
async def ensure_access_tier_columns(db):
    ...
    additions = [
        ...
        # 2026-09-20 (Craig: "does the curiosity queue retain my answers?").
        # It did not — the question was stored, the answer was not, and
        # nothing marked one as answered. She asked four of them twice and
...
async def answer_curiosity_question(topic: str, answer: str) -> bool:
    """Keeps what he said back, and stops her asking again.

    2026-09-20 (Craig: "does the curiosity queue retain my answers?"). It
    did not. She asked, he answered, and the answer went into `memory` as
    an ordinary turn with nothing linking it to the question — so it fell
    out of the four-turn window and the question stayed open until the
    ask-limit dropped it. Four of the five on file were asked twice and
    discarded, having been answered both times.

    This is also the best knowledge in the system and it was being thrown
    away: a direct answer from him to a question she chose to ask is
    grounded in the person who knows, not generated by her. The
    auto-storage that got removed today was trying to capture things like
    this and catching conversational noise instead."""
    if not answer or not answer.strip():
        return False

    async with aiosqlite.connect(DB_PATH) as db:
        cursor = await db.execute(
            "UPDATE curiosity_queue SET answer=?, answered_at=CURRENT_TIMESTAMP "
            "WHERE topic=? AND answer IS NULL",
            (answer.strip(), topic))
        await db.commit()
        return cursor.rowcount > 0


async def fetch_answered_curiosity(limit: int = 20):
    """What she asked about and was actually told."""
    async with aiosqlite.connect(DB_PATH) as db:
        cursor = await db.execute(
            "SELECT topic, question, answer, answered_at FROM curiosity_queue "
            "WHERE answer IS NOT NULL ORDER BY answered_at DESC LIMIT ?",
            (limit,))
        rows = await cursor.fetchall()
    keys = ["topic", "question", "answer", "answered_at"]
    return [dict(zip(keys, r)) for r in rows]
