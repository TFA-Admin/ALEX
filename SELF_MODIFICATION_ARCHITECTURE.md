...
- `check_safety()`'s scope system covers `os`/`network`/`db` stdlib
  imports, but never blocks importing the project's OWN modules (e.g.
  `from db.db import ...`) — a module can reach real capabilities by
  importing internal code directly, sidestepping the whole scope system.
- Per-utterance creator authority isn't real yet — voice verification is
  connect-time-once, not re-checked if the creator speaks up mid-session
  under a different user's active identity (Design Principle 9's
  example).
