...
**Still open from the swap.** The `base` → `large-v3` Whisper upgrade is the intended use of the freed VRAM and is a larger quality win than an LLM swap, but it is **blocked on provisioning**: only `models--Systran--faster-whisper-base` is present in the local HF cache, and both STT and embedding modules deliberately set `HF_HUB_OFFLINE=1`/`TRANSFORMERS_OFFLINE=1` because ALEX must never reach the network. Fetching a larger model is therefore a deliberate out-of-band step, not something the runtime should be allowed to do on its own. Reopened as genuinely-cheap-now rather than decided: the module-gap classifier (retired purely at ~2s/turn), per-utterance speaker re-verification (blocking Design Principle 9 and Component 12), and Phase 6 continuous reflection, whose entire gating scheme exists because reflection competed with live conversation. `get_preferred_model`/`log_model_usage`, kept dormant specifically to be revisited at this upgrade, are to be deleted: the counter only ever observed the default because nothing ever let a user pick a model, and per-*user* is the wrong axis — per-*task* routing would be a rewrite, not a revival. Note that `module_versions` still holds a diagnostic_tool v2 that reads `speech.stt_engine.FORCE_STT_CPU`; the live module is v3 and does not, but a rollback to v2 through the Controller's version-history UI would now fail on the deleted flag.
